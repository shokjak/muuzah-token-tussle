  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-primary font-['Orbitron']">
          Select Token
        </h3>
        <div className="text-sm text-muted-foreground">
          <span className="text-accent">{tokensPlaced}</span>/{tokensRequired} tokens
          {' • '}
          <span className="text-destructive">{bombsPlaced}</span>/{bombsRequired} bombs
        </div>
      </div>
          ...
          SHAPES.map((shape) => {
            ...
            return (
              <button
                ...
                disabled={tokensPlaced >= tokensRequired && !isSelected}
                className={cn(
                  ...
                  tokensPlaced >= tokensRequired && !isSelected && 'opacity-50 cursor-not-allowed'
                )}
              >
      ...
      <button
        ...
        disabled={bombsPlaced >= bombsRequired && !selectedBomb}
        className={cn(
          ...
          bombsPlaced >= bombsRequired && !selectedBomb && 'opacity-50 cursor-not-allowed'
        )}
      >
      ...
      <p className="text-xs text-muted-foreground text-center">
        Click a token/bomb, then click on the grid to place it
      </p>
    </div>
  );
}
